...
use gradeexport_haifa_administration_sap\GradeExport;
use gradeexport_haifa_administration_sap\Form\GradeExportForm;
...
defined('HAIFA_ADMINISTRATION_SAP') || require_once __DIR__.'/classes/Autoloader/autoload.php';
...
//
//    //$PAGE->set_url('/grade/export/haifa_administration_sap/index.php', ['id' => $id]);
//    Globals::getPage()->set_url('/grade/export/haifa_administration_sap/index.php', ['id' => $id]);
//
...
//    require_capability('gradeexport/haifa_administration_sap:view', $context);

    print_grade_page_head($courseId, 'export', 'haifa_administration_sap', get_string('export', 'gradeexport_haifa_administration_sap'));
    export_verify_grades($courseId);

    if (!empty($cfg->gradepublishing)) {
        $cfg->gradepublishing   = has_capability('gradeexport/haifa_administration_sap:publish', $context);
    }

    $actionUrl      = new moodle_url('/grade/export/haifa_administration_sap/export.php');
...
//    $id         = required_param('id', PARAM_INT);
//
//    //$PAGE->set_url('/grade/export/haifa_administration_sap/export.php', ['id' => $id]);
//    Globals::getPage()->set_url('/grade/export/haifa_administration_sap/export.php', ['id' => $id]);
//
//    //if (!$course = $DB->get_record('course', ['id' => $id])) {
//    if (!$course = Globals::getRecord('course', ['id' => $id])) {
//        print_error('nocourseid');
...
//    require_capability('gradeexport/haifa_administration_sap:view', $context);
    ...
    if (!empty($cfg->gradepublishing) && !empty($key)) {
        ////print_grade_page_head($COURSE->id, 'export', 'haifa_administration_sap', get_string('exportto', 'grades') . ' ' . get_string('pluginname', 'gradeexport_haifa_administration_sap'));
        //print_grade_page_head($COURSE->id, 'export', 'haifa_administration_sap', get_string('export', 'gradeexport_haifa_administration_sap'));
        print_grade_page_head(Globals::getCourseId(), 'export', 'haifa_administration_sap', get_string('export', 'gradeexport_haifa_administration_sap'));
    }
...
function setupHaifaAdministrationExportPage($pageUrl)
{
    ...
    Globals::getPage()->set_url('/grade/export/haifa_administration_sap/'.$pageUrl.'.php', ['id' => $id]);
...
function setupDumpPage()
{
    ...
    require_capability('moodle/grade:export', $context);
    require_capability('gradeexport/haifa_administration_sap:view', $context);
    require_capability('gradeexport/haifa_administration_sap:publish', $context);
...
function syncSap(){
    ...
    //grade_get_course_grade($userid, $course->id);
